...
auto dyn_to_variant(const P * dyn) -> V
{
...
using expr_variant = std::variant<
    std::reference_wrapper<const ExprInt>,
    ...
    void run(ref<Store>) override
    {
        ...
        if (this->file) {
            ...
            expr = eval_state->parseExprFromString(
                this->expr.value(), eval_state->rootPath(CanonPath::fromCwd(getCommandBaseDir())));
        }
...
template<typename T>
void to_json(json & j, const WithSymbols<const T> & expr)
{
...
std::enable_if_t<
    ...
    std::void_t<
        decltype(json(
            std::declval<WithSymbols<std::remove_const_t<typename std::ranges::range_value_t<S>::first_type>>>())),
...
to_json(json & j, const WithSymbols<S> & expr)
{
    auto trans = std::ranges::views::transform(
        *expr, [&expr](auto x) { return std::make_pair(std::string{expr.replace(x.first)}, expr.replace(x.second)); });
...
void to_json(json & j, const WithSymbols<ExprConcatStrings> & expr)
{
    j = {
        ...
        {"strings", expr.map([](auto x) {
             return std::ranges::views::transform(*x.es, [](auto && p) -> Expr * { return p.second; });
         })},
    };
}
